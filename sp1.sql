...
select @pfromDate='2024-01-28 00:00'
select @ptoDate='2024-01-29 23:59'
select @pgroupcallid=''
select @ptype='Date'
...
actualcallorder int, pre_actualcallorder int, change_events varchar(500),ARR_PILOT datetime,PRE_ARR_PILOT datetime,DEP_PILOT datetime,PRE_DEP_PILOT datetime,EOSP datetime,PRE_EOSP datetime,BOSP datetime,PRE_BOSP datetime,DISTANCE float ,PRE_DISTANCE float
,SPEED float,PRE_SPEED float)
...
portname varchar(150),terminalname varchar(150),vesselname varchar(200), old_callorder int, old_type char(1),old_type_desc varchar(50),
actualcallorder int, pre_actualcallorder int, change_events varchar(500),ARR_PILOT datetime,PRE_ARR_PILOT datetime,DEP_PILOT datetime,PRE_DEP_PILOT datetime,EOSP datetime,PRE_EOSP datetime,BOSP datetime,PRE_BOSP datetime,DISTANCE float ,PRE_DISTANCE float
,SPEED float,PRE_SPEED float, agencyuser char(1))
...
INSERT INTO #tmpSchedule(id, pre_id, vessel_code, voyage_code, sub_bound, terminalcode, arrdock, dep_dock, status, bound, callid, callorder, type, ARR_PILOT, DEP_PILOT, EOSP, BOSP, DISTANCE, SPEED)
SELECT t.id, MAX(s.id), s.vessel_code, s.voyage_code, s.sub_bound, s.terminalcode, s.arrdock, s.dep_dock, s.status, s.bound, s.callid, s.callorder, s.type, s.ARR_PILOT, s.DEP_PILOT, s.EOSP, s.BOSP, s.DISTANCE, s.SPEED
FROM #tmpCallid t
JOIN #temp_ScheduleModified_id s ON t.callid = s.callid AND s.id < t.id
LEFT JOIN #temp_ScheduleModified_id s2 ON t.callid = s2.callid AND s2.id < t.id AND s2.id > s.id
WHERE s2.id IS NULL
GROUP BY t.id, s.vessel_code, s.voyage_code, s.sub_bound, s.terminalcode, s.arrdock, s.dep_dock, s.status, s.bound, s.callid, s.callorder, s.type, s.ARR_PILOT, s.DEP_PILOT, s.EOSP, s.BOSP, s.DISTANCE, s.SPEED;
 
-- Delete processed records from #tmpCallid
DELETE t FROM #tmpCallid t JOIN #deleteid tm ON t.id = tm.id;
 
-- Clear the deleteid table
TRUNCATE TABLE #deleteid
